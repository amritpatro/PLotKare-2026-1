...
export default async function OwnerSectionPage({ params, searchParams }: PageProps) {
  ...
  const [{ data: owner }, { data: properties }, { data: documents }, { data: services }, { data: tickets }, { data: amenityCatalog }] = await Promise.all([
    ...
    supabase.from('properties').select('id,title,property_kind,asset_type,address,city,state,occupancy_status,lifecycle_status,verification_status,created_at').eq('owner_profile_id', user.id).order('created_at', { ascending: false }),
  ...
  if (section === 'register') {
    content = <div className="space-y-6"><Title title="Register property" body="Register a property into PlotKare verification and care operations." /><form action={registerOwnerProperty} className={`${cardClass} grid gap-3 md:grid-cols-2`}><select name="assetType" className={inputClass} defaultValue="plot"><option value="plot">Plot / vacant land</option><option value="apartment">Apartment / flat</option><option value="house">Independent house / villa</option><option value="commercial">Commercial property</option><option value="agricultural_land">Agricultural land</option><option value="mixed_other">Mixed / other</option></select><input name="title" required placeholder="Property title" className={inputClass} /><input name="address" required placeholder="Address" className={inputClass} /><input name="city" required placeholder="City" className={inputClass} /><input name="state" defaultValue="Andhra Pradesh" className={inputClass} /><input name="postalCode" placeholder="Postal code" className={inputClass} /><input name="plotNumber" placeholder="Plot / unit ID" className={inputClass} /><input name="sqYards" type="number" min="1" placeholder="Sq. yards equivalent" className={inputClass} /><PendingActionButton className={`${buttonClass} md:col-span-2`} pendingText="Submitting...">Submit for verification</PendingActionButton></form></div>
